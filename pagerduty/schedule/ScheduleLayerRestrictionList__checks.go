...
func (j *jsiiProxy_ScheduleLayerRestrictionList) validateSetInternalValueParameters(val interface{}) error {
	...
	case cdktn.IResolvable:
		// ok
	case *[]*ScheduleLayerRestriction:
		val := val.(*[]*ScheduleLayerRestriction)
	...
	case []*ScheduleLayerRestriction:
		val_ := val.([]*ScheduleLayerRestriction)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktn.IResolvable, *[]*ScheduleLayerRestriction; received %#v (a %T)", val, val)
		}
	}
