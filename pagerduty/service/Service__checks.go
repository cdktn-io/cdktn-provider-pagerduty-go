...
func (s *jsiiProxy_Service) validatePutScheduledActionsParameters(value interface{}) error {
	...
	case cdktn.IResolvable:
		// ok
	case *[]*ServiceScheduledActions:
		value := value.(*[]*ServiceScheduledActions)
	...
	case []*ServiceScheduledActions:
		value_ := value.([]*ServiceScheduledActions)
		value := &value_
		for idx_cd4240, v := range *value {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter value[%#v]", idx_cd4240) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(value) {
			return fmt.Errorf("parameter value must be one of the allowed types: cdktn.IResolvable, *[]*ServiceScheduledActions; received %#v (a %T)", value, value)
		}
	}
