...
func (j *jsiiProxy_ServiceDependencyDependencyDependentServiceList) validateSetInternalValueParameters(val interface{}) error {
	...
	case *[]*ServiceDependencyDependencyDependentService:
		val := val.(*[]*ServiceDependencyDependencyDependentService)
	...
	case []*ServiceDependencyDependencyDependentService:
		val_ := val.([]*ServiceDependencyDependencyDependentService)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktn.IResolvable, *[]*ServiceDependencyDependencyDependentService; received %#v (a %T)", val, val)
		}
	}
